...
generateKernelH0Cube <- function(piVector,p0Vector,h,kernel){
  ...
  # For example, hCube[1][2,3] gives you the kernelH value of h=[1], p-p0=[2][3]
    ...
    matrixByHIndex <- array(0,dim = c(totalPs, totalP0s))
    
    for(j in 1:totalP0s){
      
      for(k in 1:totalPs){
        
        matrixByHIndex[k,j] <- kernelHFunction(piVector[k]-p0Vector[j],h,kernel)
...
generatePolinomialCube1 <- function(piVector, p0Vector){
  ...
  for (k in 1:3) { # We only need 0,1,2,)
...
findLittleA <- function(p0,h,r,kernelCube,poliCube){
  ...
  myAr = sum(poliCube[[r]][,p0]*kernelCube[,p0])
...
findLittleAvaluesb <- function(p0Index,hIndex,kernelCube,poliCube){
