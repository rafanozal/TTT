...
# covarianceMatrix = generateCovarianceXMatrixBOOTSTRAP(myData, bootstrapSample)
  ...
  sigmaMatrix<-generateVarianceXVectorBOOTSRAP_otro(x=myData, bootFactor = 100) ###NUEVO!!!!
  varianceVector<-diag(sigmaMatrix)                                             ###NUEVO!!!!
  covarianceMatrix<-sigmaMatrix                                                 ###NUEVO!!!! 
  sigmaAux<-t(sigmaMatrix);diag(sigmaAux)<-0                                    ###NUEVO!!!! 
  sigmaMatrix<-sigmaAux+sigmaMatrix                                             ###NUEVO!!!!
 ...
 # sigmaMatrix      = generateSigmaMatrix(varianceVector, covarianceMatrix)
  ...
  if(myMethod == "quadratic"){
    ...
    #kernelZeroBarBarCube   = generateKernelBarBarHCube(kernelZeroBarCube, wMatrix)
  
    kernelFirstBarCube     = generateKernelBarHCube(kernelCube, poliCube, a0Matrix, a1Matrix, a2Matrix, a3Matrix, a4Matrix, 1)
    #kernelFirstBarBarCube  = generateKernelBarBarHCube(kernelFirstBarCube, wMatrix)
  
    kernelSecondBarCube    = generateKernelBarHCube(kernelCube, poliCube, a0Matrix, a1Matrix, a2Matrix, a3Matrix, a4Matrix, 2)
    #kernelSecondBarBarCube = generateKernelBarBarHCube(kernelSecondBarCube, wMatrix)
  ...
  for (i in 1:xgrid) { # For each of the p0 values, 401 by default
    ...
    for (j in 1:ygrid) { # For each of the hs, 11 by default
      ...
      #currentZeroVariance   = findVariancePhiWithKernel(j,i, kernelFirstBarBarCube,  varianceVector, covarianceMatrix, 0)
      #currentFirstVariance  = findVariancePhiWithKernel(j,i, kernelFirstBarBarCube,  varianceVector, covarianceMatrix, 1)
      #currentSecondVariance = findVariancePhiWithKernel(j,i, kernelSecondBarBarCube, varianceVector, covarianceMatrix, 2)

      if(myMethod == "quadratic"){

        currentZeroVariance   = findVariancePhiWithKernelNEW(j,i, kernelZeroBarCube,   sigmaMatrix, wMatrix, 0)
        currentFirstVariance  = findVariancePhiWithKernelNEW(j,i, kernelFirstBarCube,  sigmaMatrix, wMatrix, 1)
        currentSecondVariance = findVariancePhiWithKernelNEW(j,i, kernelSecondBarCube, sigmaMatrix, wMatrix, 2)

      }
      
      else{
        
        currentZeroVariance   = findVariancePhiWithKernelCubicNEW(j,i, kernelZeroBarCube,   sigmaMatrix, wMatrix, 0)
        currentFirstVariance  = findVariancePhiWithKernelCubicNEW(j,i, kernelFirstBarCube,  sigmaMatrix, wMatrix, 1)
        currentSecondVariance = findVariancePhiWithKernelCubicNEW(j,i, kernelSecondBarCube, sigmaMatrix, wMatrix, 2)
