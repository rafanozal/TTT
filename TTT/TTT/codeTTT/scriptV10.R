...
runTest <- function(dataPath, testRepetitions, blueThreshold,
  ...
  csvFileName = paste(rightNow,"_","testResult.csv", sep = "")
  runFolder   = file.path(paste(getwd(),"/results/", sep = ""))
  csvPath     = file.path(runFolder, csvFileName)
  ...
  testResults = data.frame(sampleSize=testRepetitions, min=allzeros,
  ...
  for (i in 1:totalTests){
    ...
    for (j in 1:repetitions){
      ...
      # Run the TTT
      myResult <- ttt(myData, xgrid, ygrid, kernel, myMethod, ESSLimit, bootstrapSample)
  ...
  write.csv2(testResults, file = csvPath)
...
runTest(dataPath, testRepetitions, blueThreshold, xgrid, ygrid,
        kernel, myMethod, variance, quantileMethod, ESSLimit, bootstrapSample)
