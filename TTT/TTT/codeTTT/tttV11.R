...
source("toolsV11.R",    encoding="utf-8")
...
#' 
#' @param xgrid   number of estimation points
#' 
...
#' 
#' @param quantileMethod You can choose which quantile method use to
...
#' 
#'                 How many observations do you need to have around to be a valid result. 
#'                 
...
#'                 Default is 100
#'                 
#' @param savePlots Generate all the plots and save them into the result folder (Default = TRUE)
#'                 
...
#' 
#' @param silent Don't print anything into the console. For long TTTs this may be not wise as you won't see any progress until several hours passes (Default = FALSE)
#'
#' @return The function itself, return the raw data of all the calculations in a dataframe with this columns:
#'         
...
#' 
#' 
ttt <- function(myData, xgrid = 401, ygrid = 11, hMin = 0, hMax = 1,
                kernel = "gaussian", myMethod = "quadratic",
                variance = "bootstrap",  quantileMethod = "normal", 
                alpha = 0.05, ESSLimit = 5, bootstrapSample = 100,
                savePlots = TRUE, saveCSV = TRUE, saveLog = FALSE,
                blackAndWhite = FALSE, silent = FALSE){
  ...
  # -----------------------------------------------------------------------------
  # -- Only do so if we are saving something. If saving nothing, don't create the
  # -- folder, otherwise you will end up with 1000s of empty folders named after
  # -- a timestamp.
  if(savePlots == TRUE || saveCSV == TRUE || saveLog == TRUE){
  ...
  maxHeight = hValues[h] - hValues[h-1] # ggplot will draw only lines in the h's, this serves
                                        # to fill the gap in between if you don't do the log10(h) trick
  ...
  sigmaMatrix          = generateVarianceXVectorBOOTSTRAP(x=myData, bootFactor = bootstrapSample) # This function gives you a superior triangular matrix.
  downTriangule        = t(sigmaMatrix);                                                  # The target sigma matrix is the same matrix, but simetrical
  diag(downTriangule)  = 0                                                                # with respect the main diagonal. So all the transposing
  sigmaMatrix          = downTriangule + sigmaMatrix                                      # and addig is to copy the top triangular to the button triangular
                                                                                          # and make this variable a real sigma matrix
  varianceVector       = diag(sigmaMatrix)                                             
  covarianceMatrix     = sigmaMatrix                                                 
  ...
  for (i in 1:xgrid) { # For each of the p0 values, 401 by default
    ...
    if(silent == FALSE){ print(paste(round(i/xgrid*100,2)," %")) }
    ...
    for (j in 1:ygrid) { # For each of the hs, 11 by default
      ...
      if(myMethod == "quadratic"){
        ...
        currentThetas = findThetasCubic(currentBigAs, currentLittleAs)
      }
    ...
    # Show time to go
    if(silent == FALSE){ print((xgrid-i) * deltaTime)}
  ...
  for (i in 1:xgrid) { # For each of the p0 values, 401 by default
    ...
    # Feedback to the user about how much is done and how much time is left
    if(silent == FALSE){ print(paste(i/xgrid*100," %")) }
    previosTime = Sys.time()
  ...
  if(quantileMethod == "simultaneous"){
    ...
    for (i in 1:xgrid) { # For each of the p0 values, 401 by default
      ...
      for (j in 1:ygrid) { # For each of the hs, 11 by default
        ...
        CIData$LeftIntervalZero[myRowIndex]   = CIData$LeftIntervalZero[myRowIndex] - CIData$p0[myRowIndex] 
        CIData$RightIntervalZero[myRowIndex]  = CIData$RightIntervalZero[myRowIndex] - CIData$p0[myRowIndex] 
  ...
  if(savePlots == TRUE){
    ...
    phiPlot <- ggplot(BaseData, aes(x=pis, y=phi)) +
      ...
      geom_abline(intercept = 0, slope = 0, linetype = 1, color="black") + 
      geom_abline(intercept = 1, slope = 0, linetype = 1, color = "black" ) +
    ...
    # -- SiZer 
    sizerZero <- ggplot(CIData, aes(p0, log10(h), z= ColorCodeZero)) +
    ...
    # -- SiZer with distance
    ggplot(CIData, aes(p0, log10(h), z= distanceZero)) +
    ...
    # -- SiZer 
    sizerFirst <- ggplot(CIData, aes(p0, log10(h), z= ColorCodeFirst)) +
    ...
    # -- SiZer with distance
    ggplot(CIData, aes(p0, log10(h), z= distanceFirst)) +
    ...
    # -- SiZer 
    sizerSecond <- ggplot(CIData, aes(p0, log10(h), z= ColorCodeSecond)) +
    ...
    # -- SiZer with distance
    ggplot(CIData, aes(p0, log10(h), z= distanceSecond)) +
    ...
    ggplot(CIData, aes(p0, log10(h), z= ESS)) +
