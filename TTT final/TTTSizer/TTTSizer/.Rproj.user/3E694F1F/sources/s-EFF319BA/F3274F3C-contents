...
#'                time to calculate.
#'
#' @param ygrid   how many h we are we going to generate.
#'
#'                Default is 11.
#'
...
#'
#' @param blackAndWhite Save your plots with a black and white theme (Default = FALSE)
#'
...
ttt <- function(myData, xgrid = 401, ygrid = 11, hMin = 0, hMax = 1,
                kernel = "gaussian", myMethod = "quadratic", quantileMethod = "normal",
                alpha = 0.05, ESSLimit = 5, bootstrapSample = 500,
                savePlots = TRUE, saveCSV = TRUE, saveLog = FALSE,
                blackAndWhite = FALSE){

  # Initialize basic variables
  {
    variance = "bootstrap"   # Moment version of variance is depecrated, no option in this final version
    n       = length(myData)
    h       = ygrid
  ...
  {
    print("Please wait, I'm initializing constants")
  ...
  # -----------------------------------------------------------------------------
  {
    ...
    for (i in 1:xgrid) { # For each of the p0 values, 401 by default
      ...
      for (j in 1:ygrid) { # For each of the hs, 11 by default
        ...
        # Find all thetas
        if(myMethod == "quadratic") currentThetas = findThetas(currentBigAs, currentLittleAs)
        else currentThetas = findThetasCubic(currentBigAs, currentLittleAs)
  ...
  # -----------------------------------------------------------
  {
    ...
    currentTime2 = Sys.time()
    varianceTime = currentTime2 - previosTime2
  }
  ...
  # -----------------------------------------------------------
  {
    if(quantileMethod == "simultaneous"){
      ...
      currentTime2 = Sys.time()
      confidentTime    = currentTime2 - previosTime2
  ...
  if(sum(CIData$ZeroBiggerSecond)>0){
    CIData[CIData$ZeroBiggerSecond,]$ColorCodeSecond  = "cyan"
  }
