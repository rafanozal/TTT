...
#' 
#' 
ttt.e <- function(myData, xgrid = 401, ygrid = 11, hMin = 0, hMax = 1,
  ...
  if(variance=='exponential'){
      x<-myData
      m<-mean(x)
      n<-length(x)
      aux<-1/(n:1)^2;aux<-aux/m
      
     sigmaMatrix<-matrix(0,n,n)
      diag(sigmaMatrix)<-cumsum(aux)
      for(i in 1:(n-1))
      {for(j in (i+1):n)
      {
        sigmaMatrix[i,j]<-sigmaMatrix[i,i]
      }}
  }else{
